...
// Bumble uses a message-type based API (same engine as Badoo)
const BUMBLE_API = 'https://bumble.com/mwebapi.phtml';
const BUMBLE_UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/144.0.0.0 Safari/537.36';

function buildHeaders(auth: PlatformAuthConfig) {
    return {
        'accept': 'application/json',
        'content-type': 'application/json',
        'user-agent': BUMBLE_UA,
...
async function bumbleRequest(auth: PlatformAuthConfig, body: any) {
    const headers = buildHeaders(auth);
    const res = await axios.post(BUMBLE_API, body, {
        headers,
        timeout: 15000,
    });
    return res.data;
...
export class BumbleAdapter implements PlatformAdapter {
    ...
    async validateAuth(auth: PlatformAuthConfig) {
        try {
            const data = await bumbleRequest(auth, {
                version: 1,
                message_type: 403, // SERVER_GET_USER
                server_get_user: {
                    user_field_filter: {
    ...
    async getRecommendations(auth: PlatformAuthConfig): Promise<PlatformRecommendation[]> {
        try {
            const data = await bumbleRequest(auth, {
                version: 1,
                message_type: 81, // SERVER_GET_ENCOUNTERS
    ...
    async likeUser(auth: PlatformAuthConfig, userId: string): Promise<LikeResult> {
        try {
            const data = await bumbleRequest(auth, {
                version: 1,
                message_type: 80, // SERVER_ENCOUNTERS_VOTE
                server_encounters_vote: {
                    person_id: userId,
                    vote: 2, // LIKE (1=DISLIKE, 2=LIKE, 3=SUPERLIKE)
                    vote_source: 1
                }
            });
            const matched = data?.body?.[0]?.client_vote_response?.vote_response_type === 2; // MATCH
    ...
    async getMessages(auth: PlatformAuthConfig, matchId: string): Promise<PlatformMessage[]> {
        try {
            const data = await bumbleRequest(auth, {
                version: 1,
                message_type: 102, // SERVER_OPEN_CHAT
                server_open_chat: {
                    user_id: matchId
                }
            });
    ...
    async sendMessage(auth: PlatformAuthConfig, matchId: string, text: string): Promise<SendMessageResult> {
        try {
            await bumbleRequest(auth, {
                version: 1,
                message_type: 104, // SERVER_SEND_CHAT_MESSAGE
    ...
    async unmatch(auth: PlatformAuthConfig, matchId: string) {
        try {
            await bumbleRequest(auth, {
                version: 1,
                message_type: 253, // SERVER_BLOCK_USER
                server_block_user: {
                    user_id: matchId,
                    block_type: 1
                }
    ...
    getAuthScript(): string {
        return `// 🐝 BUMBLE AUTH SCRIPT
// 1. Mở bumble.com → đăng nhập
...
(() => {
  ...
  window.fetch = async (...args) => {
    ...
    if (typeof url === "string" && url.includes("mwebapi.phtml")) {
      ...
      console.log("📦 BUMBLE SESSION JSON");
      console.log(JSON.stringify(result, null, 2));
      window.__BUMBLE_SESSION__ = result;
    }
    return _fetch(...args);
  };
})();`;
